...
import { displayEntries, entries, JournalEntry } from '../script';

// Gruppierung der Tests zur Funktion 'displayEntries'.
describe('displayEntries function', () => {
  let entriesContainer: HTMLDivElement; // Deklaration einer Variable, die den Container für Einträge speichert.
  ...
  it('sollte entries korrekt anzeigen', () => {
    ...
    entries.forEach((entry, index) => {
      const entryDiv = entriesContainer.children[index] as HTMLDivElement;

      // Überprüfung des Titels.
      const titleParagraph = entryDiv.querySelector('.title') as HTMLParagraphElement;
      expect(titleParagraph.textContent).toBe(entry.title);

      // Überprüfung des Datums.
      const dateParagraph = entryDiv.querySelector('.date') as HTMLParagraphElement;
      expect(dateParagraph.textContent).toContain(entry.date);

      // Überprüfung des Inhalts.
      const contentParagraph = entryDiv.querySelector('.content') as HTMLParagraphElement;
      ...
      const bookmarkCheckbox = entryDiv.querySelector('.bookmark-checkbox') as HTMLInputElement;
