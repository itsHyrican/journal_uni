...
import { createEntry, entries, init, JournalEntry } from '../script';
...
describe('createEntry function', () => {
  ...
  it('sollte einen neuen Eintrag generieren und ihn hinzufügen', () => {
    ...
    expect((document.getElementById('content') as HTMLTextAreaElement).value).toBe('');
    expect((document.getElementById('entryDate') as HTMLInputElement).value).toBe('');
  });
});
