// Interface zur Definition der Struktur eines Journal-Eintrags
...
export const entries: JournalEntry[] = JSON.parse(
...
export function createEntry(): void {
  ...
  const contentTextarea: HTMLTextAreaElement = document.getElementById("content") as HTMLTextAreaElement;
  const entryDateInput: HTMLInputElement = document.getElementById("entryDate") as HTMLInputElement;
...
export function displayEntries(loadMore: boolean = false): void {
  // Greift auf das HTML-Element zu, das die Journal-Einträge enthält.
  const entriesContainer: HTMLElement = document.getElementById("entries")!;
  ...
  entriesToShow.forEach((entry) => {
    ...
    const titleParagraph: HTMLElement = document.createElement("p");
    titleParagraph.classList.add("title");
    titleParagraph.textContent = entry.title;

    const dateParagraph: HTMLElement = document.createElement("p");
    ...
    const contentParagraph: HTMLElement = document.createElement("p");
    ...
    if (entry.content.length > 500) {
      ...
      const moreLink: HTMLSpanElement = document.createElement("span");
    ...
    const bookmarkCheckbox: HTMLInputElement = document.createElement("input");
    ...
    const bookmarkLabel: HTMLLabelElement = document.createElement("label");
...
export function updateFilterButton(): void {
  const filterButton: HTMLElement = document.getElementById("filterButton")!;
